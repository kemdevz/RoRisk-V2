...
const ItemCard = memo(function ItemCard({ item, selected, disabled, currency, onSelect }) {
  ...
  return <div className={`upgrader-item-element element-${tier}${selected ? ' element-selected' : ''}${disabled ? ' is-disabled' : ''}`} role="button" tabIndex={disabled ? -1 : 0} aria-disabled={disabled} onClick={activate} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') activate() }} {...itemScope}><div className={`inner-selected-icon${selected ? ' is-visible' : ''}`} {...itemScope}><SelectedCheck /></div><div className="inner-vector-container" {...itemScope}><Pattern /></div><div className="inner-image" {...itemScope}><div className={`inner-glow element-${tier}`} {...itemScope} /><img src={item.image} alt={item.name} {...itemScope} /></div><div className="inner-info" {...itemScope}><div className="inner-name" title={item.name} {...itemScope}>{item.name}</div><div className="inner-price" {...itemScope}><img src={currencyIcon(currency)} alt={currency === 'rocoins' ? 'RoCoins' : 'Coins'} {...itemScope} /><div className="price-value" {...itemScope}>{formatAmount(itemValue(item, currency))}</div></div></div></div>
})
