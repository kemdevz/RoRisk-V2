...
function Chat({ onToggle, user }) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.addEventListener('message', (event) => {
        ...
        if (payload.type === 'blackjackBet' && payload.game) window.dispatchEvent(new CustomEvent('rorisk:blackjack-bet', { detail: { game: payload.game } }))
  ...
  const rainClaimPhase = Boolean(rain && synchronizedNow >= rainStartsAt)
  const rainTimer = rain
    ? rainClaimPhase
      ? Math.max(0, Math.ceil((rainEndsAt - synchronizedNow) / 1000))
      : Math.max(0, Math.floor((rainStartsAt - synchronizedNow) / 1000))
    : 30 * 60
